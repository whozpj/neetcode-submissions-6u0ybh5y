...
#focus here, kinda aids to get this first try
    def put(self, key: int, value: int) -> None:
        ...
        #                   this to end
            ...
            #IMPORANTTTTT: we need to make sure that we delete from the hashmap
            # before we remove from the linkedlist. This is because if we remove
            # self.leftMost.next first, self.leftMost.next changes to the next
            # value in the list as the original node we target is removed
            del self.hashmap[self.leftMost.next.key]
            self.remove(self.leftMost.next)
            
            # add to mao, and update list
            self.hashmap[key] = newNode
            self.insert(newNode)
        else:
            #final case, under capacity and new node
            newNode = Node(value,key)
            self.hashmap[key] = newNode
            self.insert(newNode)
